      ...
      <div className="w-full max-w-[440px] z-10 animate-fade-in">
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-brand-gold rounded-2xl shadow-2xl shadow-brand-gold/20 mb-6 transform hover:rotate-12 transition-transform">
            <span className="text-white text-3xl font-bold">{storeName ? storeName.charAt(0).toUpperCase() : 'L'}</span>
          </div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Welcome Back</h1>
          <p className="text-text-muted mt-2">Sign in to manage your {storeName} store</p>
        </div>

        <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-[32px] shadow-2xl">
          {error && (
            <div className="mb-6 p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl text-rose-500 text-sm font-bold text-center animate-fade-in">
              {error}
            </div>
          )}
